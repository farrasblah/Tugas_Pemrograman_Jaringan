...
def ProcessTheClient(connection, address):
    try:
        ...
        logging.warning(f"Error during client handling: {e}")
    finally:
        connection.close()
...
def Server():
    ...
    my_socket.bind(('0.0.0.0', 8885))  # gunakan port 8885 untuk thread pool
    my_socket.listen(10)
    print("Thread pool HTTP server listening on port 8885...")
    ...
    with ThreadPoolExecutor(20) as executor:
        while True:
            ...
            aktif = ['x' for i in the_clients if i.running()]
            print(f"Jumlah thread aktif: {len(aktif)}")
